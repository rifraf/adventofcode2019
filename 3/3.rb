...
#===========================================================
def matching_points(path1, path2)
  path1_locations = trace_locations(path1)
  path2_locations = trace_locations(path2)

  path1_locations & path2_locations # Intersection
end

#===========================================================
def trace_locations(path)
  grid = []
  x = y = 0
  path.each do |step|
    break unless step =~ /(\S)(\d+)/
    direction = $1
    length = $2.to_i
    case direction
    when 'R'
      length.times { |l| grid << [x + l, y] }
      x += length
    when 'L'
      length.times { |l| grid << [x - l, y] }
      x -= length
    when 'U'
      length.times { |l| grid << [x, y + l] }
      y += length
    when 'D'
      length.times { |l| grid << [x, y - l] }
      y -= length
...
class Examples2 < Test::Unit::TestCase
  ...
  def test_example1
    ...
    assert_equal 15, path_length_to(p2, [6, 5])
  end
  ...
  def test_example2
    ...
    result = matches.map do |m|
      path_length_to(p1, m) + path_length_to(p2, m)
    end.min
    assert_equal 610, result
  end
  ...
  def test_example3
    ...
    assert_equal 410, result
  end
end

#===========================================================
def path_length_to(path, point)
  path_length = 0
  x = y = 0
  lengths = []
  path.each do |step|
...
#===========================================================
matches = matching_points(path1, path2)
result = matches.map do |m|
  path_length_to(path1, m) + path_length_to(path2, m)
end.min
puts "Part 2 result = #{result}" # <- 4158
