...
class Examples16a < Test::Unit::TestCase
  ...
  def test_example2
    ...
    patterns[0] = [0, 1, 0, -1]
    (1..7).each do |i|
      patterns[i] = patterns[0].map { |n| [n] * (i + 1) }.flatten
    end
    patterns.each { |p| p.rotate! }
    ...
    # Phase 1
    result = fft_phase(signal, seed)
    assert_equal [4, 8, 2, 2, 6, 1, 5, 8], result

    # Phase 2
    result = fft_phase(result, seed)
...
#===========================================================
def fft_phase(signal, seed)
  output = []
  signal.length.times do |i|
    pattern = seed.map { |n| [n] * (i + 1) }.flatten.rotate
    items = signal.map { |s| pattern.rotate!; s * pattern.last }
    output << items.reduce(&:+).abs % 10
  end
  output
end
